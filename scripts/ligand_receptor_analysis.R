...
get_ligand_receptor_data=function(  reg=1e-6,exprs_thresh=5e-6){
  inf_pat1=inflamed_samples[sample_to_patient[inflamed_samples]%in%pat1]
  uninf_pat1=uninflamed_samples[sample_to_patient[uninflamed_samples]%in%pat1]

  inf_pat2=inflamed_samples[sample_to_patient[inflamed_samples]%in%pat2]
  uninf_pat2=uninflamed_samples[sample_to_patient[uninflamed_samples]%in%pat2]

  samps<<-intersect(as.character(c(inf_pat1,inf_pat2,uninf_pat1,uninf_pat2)),c(inflamed_samples_v2,uninflamed_samples_v2))


    a=read.csv("input/Gene lists//Ligands_receptors_pairs.csv",stringsAsFactors = F)[,1:5]
    a=rbind(a,"-")
    a=apply(a,2,function(x){adjust_gene_names(paste(x,collapse = ","),rownames(ileum_ldm$dataset$umitab))})
    a=a[-nrow(a),]
    get_one=function(i){
      mask=a[,i]!=""
  ...
  genes=rownames(ileum_ldm$model$models)
  validated_pairs<<-validated_pairs[validated_pairs[,1]%in%genes&validated_pairs[,2]%in%genes,]
  subtype_models=sapply(split(as.data.frame(t(ileum_ldm$model$models)),cluster_to_subtype1[colnames(ileum_ldm$model$models)],drop=F),colMeans)
  
  cc2<<-unique(validated_pairs[,1])
  corrected_counts<<-pmax(ileum_ldm$dataset$counts-ileum_ldm$dataset$noise_counts,0)
  ...
  counts_inf_pat1=apply(corrected_counts[as.character(inf_pat1),,],2:3,sum)
  counts_inf_pat2=apply(corrected_counts[as.character(inf_pat2),,],2:3,sum)
  counts_uninf_pat1=apply(corrected_counts[as.character(uninf_pat1),,],2:3,sum)
  counts_uninf_pat2=apply(corrected_counts[as.character(uninf_pat2),,],2:3,sum)
  ...
  freqs=normalize_by_clusterset_frequency(ileum_ldm$dataset,samples = samps,cluster_sets=ileum_ldm$cluster_sets,pool_subtype = T,reg = 0)
  freqs<<-do.call(cbind,freqs)

  m_rec2=ileum_ldm$model$models[validated_pairs[,2],]
...
get_corr_map=function(){
  cor_one_cell_type=function(subtype,method="spearman",reg=5e-7){
    cor(freqs[,subtype],log2(reg+((total_counts_per_gene_per_patient[,rownames(m_cc2_normed)]-t(counts_per_gene_per_subtype_per_patient[subtype,rownames(m_cc2_normed),]))/unlist(total_counts_per_patient-total_counts_per_subtype_per_patient[subtype,]))[samps,]),method=method)
  }
  
  subtypes=colnames(freqs)
  cormat_freq_X_cyt_expression=t(sapply(subtypes,cor_one_cell_type))
  
  colnames(cormat_freq_X_cyt_expression)=rownames(m_cc2_normed)
  cormat_freq_X_cyt_expression<<-t(cormat_freq_X_cyt_expression)
}
  
  
#cluster_to_subtype1=cluster_to_subtype
#cluster_to_subtype1[cluster_to_subtype1%in%c("Naive T cells","Central Memory T cells")]="Naive/CM T cells"
main_ligand_receptors=function(){
  get_ligand_receptor_data()
  get_corr_map()
  celltype_ord_freq=rev(colnames(cormat_freq_X_cyt_expression)[get_order(seriate(as.dist(1-cor(cormat_freq_X_cyt_expression,use="comp")),"GW"))])

  open_plot("output/main_figures/",fn = "figure_3h_ligand_ligand_subtype_freqs_correlation",plot_type = "pdf",nrow(m_cc2_normed)/6,height =2+length(celltype_ord_freq)/8)
  par(mar=c(6,1,1,15))
  image(cormat_freq_X_cyt_expression[rownames(m_cc2_normed),celltype_ord_freq],col=colorRampPalette(c("blue","white","red"))(100),breaks=c(-1,seq(-1,1,l=99),1),axes=F)
  mtext(text = rownames(m_cc2_normed),side = 1,at = seq(0,1,l=nrow(m_cc2_normed)),las=2,cex=.7,line=.5)
  mtext(text = celltype_ord_freq,side = 4,at = seq(0,1,l=ncol(cormat_freq_X_cyt_expression)),las=2,cex=.7,line=.5)
  close_plot()
  
  open_plot(path = "output/main_figures/",fn="figure_3h_ligand_ligand_subtype_freqs_correlation",plot_type = "png",width = 200,height =50)
  par(mar=c(2,1,0,1))
  ...
  receptor_ord=get_order(seriate(as.dist(1-cormat_genes_rec),method = "GW"))
  m_rec2_normed<-m_rec2_normed[receptor_ord,]
  open_plot("output/main_figures/",fn = "figure_3c_rec_expression",plot_type = "pdf",nrow(m_cc2_normed)/6,height =2+length(celltype_ord_freq)/8)
  ...
  open_plot("output/main_figures/",fn = "figure_3b_ligand_expression",plot_type = "pdf",nrow(m_cc2_normed)/6,height =2+length(celltype_ord_freq)/8)
  ...
  open_plot(path = "output/main_figures/",fn="figure_3b_ligand_expression",plot_type = "png",width = 200,height =50)
  par(mar=c(2,1,0,1))
  image(matrix(seq(-4,4,l=100),,1),col=colgrad_rel,axes=F,breaks=seq(-4,4,l=101),)
  axis(1,at=seq(0,1,l=5),labels=F,lwd = 2)
  close_plot()
  ...
  ligands_to_highlight_tab=read.table("input/tables/ligands_to_highlight.txt",sep="\t",header=T,stringsAsFactors = F)
  ligands_to_highlight=strsplit(ligands_to_highlight_tab[,1],",")
  ligands_to_highlight_cols=ligands_to_highlight_tab[,2]
  open_plot("output/main_figures/",fn = "figure_3bc_connectors",plot_type = "pdf",width =nrow(m_cc2_normed)/6,height =(2+length(celltype_ord_freq)/8)/3)
  ...
  g=t(total_counts_per_gene_per_patient/rowSums(total_counts_per_gene_per_patient))
  gcc2=g[cc2,c(names(sample_to_patient[inflamed_samples][match(c(pat1,pat2),sample_to_patient[inflamed_samples])]),control_samples)]
  reg=1e-6
  lgcc2=log2((reg+gcc2)/(reg+rowMeans(gcc2)))
  cormat=cor(t(lgcc2))
  ord=get_order(seriate(as.dist(1-cormat),method="GW"))
  open_plot("output/main_figures/",fn = "figure_total_ligand_per_sample",plot_type = "pdf",width = 5,10)
  image(t(lgcc2[ord,]),col=colorRampPalette(c("blue","white","red"))(100),axes=F,breaks=c(-200,seq(-2,2,l=99),200))
  mtext(text = colnames(lgcc2),side = 1,at = seq(0,1,l=ncol(lgcc2)),las=2,cex=.5)
  mtext(text = colnames(cormat)[ord],side = 2,at = seq(0,1,l=ncol(cormat)),las=2,cex=.5)
  close_plot()

#tclusts=names(cluster_to_cluster_set)[cluster_to_cluster_set=="T"]
  tclusts=c("21","26","4","47","34","30","46","42")
  t_total_counts_per_gene_per_patient<<-apply(corrected_counts[,,tclusts],1:2,sum)
  g=t(t_total_counts_per_gene_per_patient/rowSums(t_total_counts_per_gene_per_patient))
  gcc2=g[cc2,c(names(sample_to_patient[inflamed_samples][match(c(pat1,pat2),sample_to_patient[inflamed_samples])]),control_samples)]
  reg=1e-5
  lgcc2=log2((reg+gcc2)/(reg+rowMeans(gcc2)))
  lgcc2=lgcc2[apply(lgcc2,1,var)>0,]
  cormat=cor(t(lgcc2))
  ord=get_order(seriate(as.dist(1-cormat),method="GW"))
  open_plot("output/main_figures/",fn = "figure_tcells_total_ligand_per_sample",plot_type = "pdf",width = 5,10)
  image(t(lgcc2[ord,]),col=colorRampPalette(c("blue","white","red"))(100),axes=F,breaks=c(-200,seq(-2,2,l=99),200))
  mtext(text = colnames(lgcc2),side = 1,at = seq(0,1,l=ncol(lgcc2)),las=2,cex=.5)
  mtext(text = colnames(cormat)[ord],side = 2,at = seq(0,1,l=ncol(cormat)),las=2,cex=.5)
  close_plot()
  ...
  g_pat1=rowMeans(g[cc2,names(sample_to_patient[inflamed_samples][match(pat1,sample_to_patient[inflamed_samples])])])
  g_pat2=rowMeans(g[cc2,names(sample_to_patient[inflamed_samples][match(pat2,sample_to_patient[inflamed_samples])])])
  g_ctrl=rowMeans(g[cc2,control_samples])
  m=log2(1e-6+cbind(g_pat1,g_pat2,g_ctrl))
  m=m-rowMeans(m)
  km=kmeans((m),6)
  ord=order(km$cluster)
  image(t(m[ord,]),col=colorRampPalette(c("blue","white","red"))(100),axes=F,breaks=c(-200,seq(-2,2,l=99),200))
  mtext(text = c("Pat1","Pat2","Ctrl"),side = 1,at = seq(0,1,l=3),las=2,cex=.5)
  mtext(text = rownames(m)[ord],side = 2,at = seq(0,1,l=nrow(m)),las=2,cex=.5)
  ...
  total_umi_frac1=apply(corrected_tot_mean[inflamed_samples_v2[sample_to_patient[inflamed_samples_v2]%in%pat1],,],2:3,mean)
  total_umi_frac2=apply(corrected_tot_mean[inflamed_samples_v2[sample_to_patient[inflamed_samples_v2]%in%pat2],,],2:3,mean)
  ...
  n_patient_expressing1<<-colSums(n_expressed_pooled[inflamed_samples_v2[sample_to_patient[inflamed_samples_v2]%in%pat1],,]>0)
  n_patient_expressing2<<-colSums(n_expressed_pooled[inflamed_samples_v2[sample_to_patient[inflamed_samples_v2]%in%pat2],,]>0)
...
DE_ligand_receptor_interactions=function(ldm,mask_pat2_ligands,mask_pat1_ligands,clusters_ligands,mask_pat2_clusters_receptors,mask_pat1_clusters_receptors,ligands,receptors,nmin_umi_thresh=0,nmin_cells_with_min_umi=20,reg=1e-14,nchunks=100,n_per_chunk=1000,noise_correction=F){
  ...
  if (sum(gene_mask_ligands)>0){
    ...
    if (sum(gene_mask_receptors>0)){
      ...
      message("Testing ",sum(gene_mask_ligands)," ligands and ",sum(gene_mask_receptors)," receptors")
...
get_two_subtypes_interactions=function(subtype1,subtype2,rec_thresh=1e-5,lig_thresh=1e-6,n_expressed_thresh=1){
  reg=1e-14
 ...
 # z1=log10(reg+matrix(rec_crit1,nrow(m1_rec),nrow(total_umi_frac_per_celltype_ligands1))*matrix(total_umi_frac_per_celltype_ligands1[,subtype1],nrow(m1_rec),nrow(total_umi_frac_per_celltype_ligands1),byrow = T))*validated_pairs_matrix
#  z2=log10(reg+matrix(rec_crit2,nrow(m2_rec),nrow(total_umi_frac_per_celltype_ligands2))*matrix(total_umi_frac_per_celltype_ligands2[,subtype1],nrow(m2_rec),nrow(total_umi_frac_per_celltype_ligands2),byrow = T))*validated_pairs_matrix
  ...
  if (!dir.exists("output/tables/DE_patterns/")){
    dir.create("output/tables/DE_patterns/")
  }
  ...
  if (!is.null(de_res)){
    write.csv(de_res[order(de_res$log2_FC),],file=paste("output/tables/DE_patterns//DE_inf_pat1_vs_pat2_",subtype1,"_",subtype2,".csv",sep=""))
  }
...
bin_plot2=function(subtype1,subtype2,mat,ord1,ord2,stats,fdr_thresh=1e-2){
  xlab=paste(subtype2,"Receptors")
  ylab=paste(subtype1,"Ligands")
  open_plot("output/main_figures/",paste("bin_lr",subtype1,subtype2,sep="_"),"pdf",.9+.2*nrow(mat),.9+.2*ncol(mat),2)
  par(mar=c(6,6,1,1))
...
st=c("Macrophages","Fibroblasts","DC","T","Endothelial")
intensity_score1=c()
intensity_score2=c()
pair_mask=c()
stats=c()
...
for (i in 1:(length(st))){
  ...
  for (j in (1:length(st))){
    ...
    res=get_two_subtypes_interactions(st[i],st[j])
    res_l[[i]][[j]]=res
    stats=rbind(stats,res$stats_mat)
...
stats$adj.p.value=p.adjust(stats$p.value)
for (i in 1:(length(st))){
  for (j in (1:length(st))){
    if (i==j){
      next
    ...
    res=res_l[[i]][[j]]
    bin_plot2(st[i],st[j],res$plot_list[["mat"]],res$plot_list[["ord1"]],res$plot_list[["ord2"]],stats)
...
open_plot("output/supp_figures/","figure_s7a_interaction_scores_distribution.png",plot_type = "pdf",width = 5,height = 4)
par(mar=c(6,5,1,1))
barplot(tab,beside=T,xlab="",ylab="log10(#pairs)",names.arg=seq(-14,-6.5,.5),ylim=c(0,3),las=2,col=c("gray",2),border=F,cex.axis = 1.2,cex.names = 1.2,space = c(0,.5))
#legend("topright",col=c("gray",2),c("Lowly expressed Receptor/Ligand","Ligand & Receptor expression detected"),pch=15,cex=.5)
close_plot()
write.csv(stats,file="output/tables/ligand_receptor_stats.csv",row.names = T,quote = F)

#bin_plot("DC","Fibroblasts")
